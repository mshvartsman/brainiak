...
def isfc_matrixlog(D, collapse_subj=True, return_p=False,
                   num_perm=1000, two_sided=False, random_state=0):
    """Intersubject functional correlation
    ...
    for p in range(n_perm + 1):
        ...
        for loo_subj in range(D.shape[2]):
            group = np.mean(D[:, :, np.arange(n_subj) != loo_subj], axis=2)
            subj = D[:, :, loo_subj]
            ISFC[:, :, loo_subj, p] = logm(np.eye(n_vox) + compute_correlation(group, subj))
    ...
    if return_p:
        p = p_from_null(ISFC, two_sided)
        return expm(ISFC[..., 0] - np.eye(n_vox)), p
    else:
        return expm(ISFC[..., 0] - np.eye(n_vox))
